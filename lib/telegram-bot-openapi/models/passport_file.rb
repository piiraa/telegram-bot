  ...
  class PassportFile
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TelegramBotOpenapi::PassportFile` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TelegramBotOpenapi::PassportFile`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'file_date')
        self.file_date = attributes[:'file_date']
      else
        self.file_date = nil
      end
    end
    ...
    def list_invalid_properties
      ...
      if @file_date.nil?
        invalid_properties.push('invalid value for "file_date", file_date cannot be nil.')
      end
    ...
    def hash
      [file_id, file_unique_id, file_size, file_date].hash
    end
