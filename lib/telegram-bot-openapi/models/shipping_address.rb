  ...
  class ShippingAddress
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TelegramBotOpenapi::ShippingAddress` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TelegramBotOpenapi::ShippingAddress`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @street_line2.nil?
        invalid_properties.push('invalid value for "street_line2", street_line2 cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @street_line2.nil?
    ...
    def hash
      [country_code, state, city, street_line1, street_line2, post_code].hash
    end
