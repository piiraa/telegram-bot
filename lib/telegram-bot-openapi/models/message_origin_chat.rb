  ...
  class MessageOriginChat
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TelegramBotOpenapi::MessageOriginChat` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TelegramBotOpenapi::MessageOriginChat`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @sender_chat.nil?
        invalid_properties.push('invalid value for "sender_chat", sender_chat cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @sender_chat.nil?
      true
    end
    ...
    def hash
      [type, date, sender_chat, author_signature].hash
    end
